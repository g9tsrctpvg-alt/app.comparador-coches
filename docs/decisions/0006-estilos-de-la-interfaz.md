...
directa —adoptar el mismo framework de utilidades que usaba el artefacto
original— es una **dependencia nueva**, y `docs/proceso/estilo.md` §1 las
somete a decisión explícita 🟡. Dos: el sitio de donde salen el color, el
espaciado y la tipografía determina si el diseño se puede revisar en un
diff o queda esparcido por el marcado.
...
- **Tailwind CSS (o cualquier framework de utilidades).** Es, con toda
  probabilidad, lo que usaba el artefacto original —es el estilo por defecto
  de un artefacto React de un solo fichero—, así que la migración sería
  literalmente copiar cadenas de clases. **Se descarta** por dos razones que
  se suman: es una dependencia nueva con su paso de build y su fichero de
  configuración, justo lo que `estilo.md` §1 obliga a decidir aparte; y
  traslada el diseño al marcado, de modo que cambiar un espaciado en toda la
  aplicación pasa a ser una búsqueda y reemplazo por los `.tsx` en vez de una
  línea en un fichero de tokens. El coste de traducir las clases del
  artefacto **una vez** es menor que el de arrastrar la dependencia y su
  modelo mental durante el resto del proyecto.
...
- **Estilos en línea (`style={{ … }}`).** **Se descarta por incapaz**, no por
  gusto: el atributo `style` no admite media queries, ni `:hover`, ni
  `:focus-visible`. Con él, ni el diseño responsive ni un indicador de foco
  accesible son expresables, y ambos son requisitos de la fase 4.
...
- Los puntos de ruptura, el modo oscuro y `prefers-reduced-motion` son media
  queries de CSS normales, sin traducción de por medio.

**Se pierde:**

- Hay que **traducir a mano** el diseño del artefacto original en vez de
  copiar sus clases. Es trabajo real, y se paga una vez.
...
| Aplazado | Disparador |
| --- | --- |
| Un selector de tema en la interfaz (claro/oscuro/sistema) | Que alguien quiera elegir tema en vez de heredar el del sistema. Respetar `prefers-color-scheme` no lo necesita |
