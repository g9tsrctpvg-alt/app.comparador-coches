...
| `product/0038` y `product/0039` en `draft` y sin decisiones abiertas, esperando el gate humano | *Propuestas de evolución* |
...
| P17 | **El consumo del enchufable presupone que se carga en casa**: el eje `coste` lee el consumo WLTP ponderado de un `PHEV`, que sale de empezar cada trayecto con la batería llena, mientras el supuesto por defecto de la aplicación es `cargaEnCasa: false` y el perfil declarado son viajes largos | `en spec` | **Redactada como `product/0038`** el 2026-09-06, en `draft` y sin decisiones abiertas: espera el gate humano. Detectada el 2026-09-06 sobre el catálogo real. El Tucson PHEV declara `consumption` 2,7 l/100 km con fuente «km77, consumo WLTP combinado», y con esa cifra va **2.º con 60,12 %, a 0,35 pp del líder**. Medido con el propio `scoreCatalog` y los pesos por defecto, sustituyendo solo esa cifra: a 5,5 l/100 km sigue 2.º con 59,63 %; a 6,6 baja a **3.º** con 58,66 %; a 7,5, a **4.º** con 57,65 % —esas tres cifras son sensibilidad, no dato: ninguna tiene fuente todavía—. Hay además una asimetría estructural: `diario` resta −1,5 al eléctrico por no cargar en casa, y el enchufable, cuyo consumo homologado depende enteramente de cargar, no paga nada. La magnitud que falta es el **consumo en modo sostenido** (batería agotada), que las fichas publican y que hoy son dos datos —el `PHEV` publicado y el despublicado, el BMW X1 xDrive25e—. La spec elige el primero de los dos caminos posibles —que `coste` use la cifra sostenida cuando `cargaEnCasa` está desactivado— y deja el segundo, un factor de utilización derivado de `electricRangeKm`, fuera de alcance con su motivo: `cargaEnCasa` es un booleano, no un patrón de uso |
| P18 | **El espacio de atrás, medido en vez de inferido**: `habitabilidad` puntúa la batalla como proxy del sitio de quien va detrás, y el propio doc de estado reconoce que reparte entre habitáculo y vanos | `en spec` | **Redactada como `product/0039`** el 2026-09-06, en `draft` y sin decisiones abiertas: espera el gate humano. Detectada el 2026-09-06. La ficha de *mediciones propias* de km77 —**la misma fuente que el catálogo ya usa** para la anchura de hombros— publica en la segunda fila «Distancia del respaldo al respaldo delantero», «Altura al techo», «Anchura hombros máxima/mínima» e «Isofix. Distancia entre anclajes», y en el maletero anchura, profundidad y «Altura de borde de carga»; comprobado fila a fila sobre la ficha del Kia EV3. Sustituir la batalla por la medida directa no pide ninguna fuente nueva —la página ya se abre coche a coche para otro dato—, y que la tabla existe para los veintiún registros no es una apuesta: los veintiuno ya citan esa misma ficha para la anchura de hombros, comprobado registro a registro. **Los anclajes quedaron fijados al redactar la spec**, midiendo once modelos en esa misma fila el 2026-09-06: el 10 en 810 mm (BMW i7 xDrive60, el mismo modelo que ya ancla la batalla, con el Škoda Superb a un centímetro) y el 0 en 590 mm (Toyota Aygo X, el más justo de los siete urbanos medidos). **Lo que la propuesta daba por hecho y resultó falso** es que esa tabla cerrara de paso la deuda de hombros mezclados: km77 publica «mínima» **o** «máxima» según el modelo, nunca las dos —la ficha del Picanto solo trae 126 cm mínima y la de la Clase E solo 146 cm máxima—, así que la deuda no se cierra y la spec se limita a hacer visible qué fila hay detrás de cada número. La altura al techo atrás queda fuera por el mismo defecto |
