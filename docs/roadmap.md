...
| `product/0014` tiene **50 de las 60 fotos posibles** (`check:photos` en verde). El **interior está completo** en los doce, y `toyota-corolla-cross`, `mazda-cx-5` y `hyundai-kona-electrico` tienen las cinco. Faltan **maletero** en nueve (`kia-ev3`, `hyundai-kona-hev`, `bmw-x1-xdrive25e`, `alfa-romeo-tonale`, `kia-sportage-hev`, `honda-civic-e-hev`, `lexus-nx-350h`, `honda-cr-v-e-hev`, referencia `alfa-romeo-giulietta`) y **lateral** en `lexus-nx-350h`. `product/0016` desbloqueó la política y ya ha dado tres fotos (Hyundai Newsroom y el 360º de Honda España), pero **el cuello de botella pasó a ser el acceso**: el navegador headless de este entorno **no tiene salida a internet** —falla incluso contra `example.com`—, así que ninguna galería montada con JavaScript (Kia, Honda newsroom, concesionarios) se puede leer; Stellantis responde 403; `bmw.es` no resuelve; y la página del NX en Lexus España sirve imágenes del **UX**, otro modelo | 2026-08-06, revisada el 2026-08-07 | Cerrar las diez que faltan. Necesita o un navegador con red, o que una persona pegue las URLs de las galerías |
