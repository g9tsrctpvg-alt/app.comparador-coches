...
| `product/0014` — la ficha del modelo y sus fotos | `superseded` por `product/0018` + `product/0023` — 2026-09-04, por decisión del usuario. Su composición ya no existe: la ficha se fundió y la tabla se retiró por debajo de `--bp-columna`. Lo que sigue vigente (esquema de foto, `photoSrc`, ficha por bloques) se lee en los docs de estado |
...
| `technical/0006` — menú de navegación: pastillas y selector en móvil | `closed` — 2026-09-04, por el ADR 0013. Dos de sus tres criterios abiertos resultaron cumplidos (la rama documentada de la disyunción del primitivo, y el despliegue, comprobado contra el sitio público); el tercero, el `overflow-x` de la cabecera a 320px, se queda sin marcar y vive como aplazamiento con disparador |
...
| `technical/0014` — el estado terminal de una spec que no se va a verificar | `consolidated` |
...
| Implementar, verificar y consolidar `technical/0014`, y cerrar `technical/0006` | Hecha — 2026-09-04. El estado `closed` y su aviso C viven en `ciclo-de-spec.md` §2 y §6 y en `consolidacion.md` §2 y §3, con el validador y sus tests detrás; `technical/0006` quedó cerrada con un único criterio sin marcar, aplazado con disparador |
...
| **Cerrada.** `product/0014` se quedaba en `implemented`, no `verified`: su criterio «a 320px se leen enteras dos columnas de modelo» **no se puede cumplir tal como está escrito** con los anchos vigentes —columna de modelo a `11rem` (176px) más la fijada, 352,75px, no caben en los 288px útiles que deja el relleno de página a 320px de viewport—, una cuenta que nadie había hecho con números reales mientras el criterio esperaba «revisión visual». No es una regresión de la fase 5: la corrección que sí trajo esa fase (`scroll-padding-left`, ver commit `c505880`) evita que la columna adyacente quede completamente tapada por la fijada, pero no la hace caber entera. El resto de sus criterios ya está verificado. **Revisado el 2026-09-04: el criterio ya no describe la interfaz que existe.** `product/0023`, consolidada el 2026-08-19, sustituyó la tabla por debajo de `--bp-columna` por la vista de un candidato a la vez, y `docs/estado/interfaz.md` lo dice en presente: «la tabla solo se renderiza visible a partir de `--bp-columna`». A 320px ya no hay ninguna columna de modelo que leer entera, ni una ni dos. El criterio no está incumplido: está **obsoleto**, y la deuda deja de ser «cómo hacer que quepan dos columnas» para ser «qué estado le corresponde a una spec cuyo criterio mide algo que el producto ya retiró» | 2026-08-12, revisada y **cerrada** el 2026-09-04 | Ninguna — cerrada. Por decisión del usuario, `product/0014` pasa a `superseded` por `product/0018` + `product/0023`: no se verifica un criterio que mide una composición retirada, y lo que la spec aportó y sigue vigente ya se lee en los docs de estado |
| **Cerrada.** `technical/0006` se quedaba en `implemented`, no `verified`: dos criterios no se cumplen tal como están escritos —(a) `overflow-x: auto` no sale de `AppHeader.module.css:.inner`, sigue haciendo falta a 320px (marca + `<select>` suman 407px sobre 320px disponibles) y justo en el borde de 592px (le faltan 31px); (b) el grupo de pastillas no compone `.buttonGhost`/`.buttonSolid`: la pastilla activa necesita un fondo persistente que `.buttonGhost` fija a `transparent` en su propia regla, y la composición entre ficheros resuelve los conflictos a favor del primitivo (hallazgo de `technical/0005`), así que ese fondo perdería siempre—, más el despliegue real, en la misma situación que las specs anteriores de esta fase. El documento en sí no se desplaza en horizontal a ningún ancho —eso sí queda resuelto—, y el resto de sus criterios está verificado. **Revisado el 2026-09-04: de los tres criterios abiertos, dos se pueden cerrar sin tocar código.** (1) El de composición del primitivo está escrito como disyunción —«…**o**, si el contenedor necesita algo que ningún primitivo actual cubre, la diferencia queda documentada aquí, no en el código sin más»— y la propia spec documenta esa diferencia por extenso, con el hallazgo de `technical/0005` detrás: su segunda rama se cumple, y dejarlo sin marcar fue un error de contabilidad, no un incumplimiento. (2) El del despliegue esperaba un `push` a `main` que desde entonces ha ocurrido decenas de veces: comprobado el 2026-09-04 contra el sitio público, que sirve `index-DEvpBliU.js` e `index-gCKKUTLA.css` —los mismos hashes que produce `npm run build`— con `200` en los dos. Queda **un solo criterio realmente incumplido**: la cabecera sigue necesitando `overflow-x: auto` a 320px | 2026-08-12, revisada y **cerrada** el 2026-09-04 | Ninguna — cerrada. Los dos criterios cumplidos se marcaron con su evidencia; el `overflow-x` pasó a *Aplazamientos con disparador* y el límite quedó escrito en `docs/estado/interfaz.md`; la spec pasó a `closed` por el ADR 0013 |
...
| El `overflow-x: auto` del carril de la cabecera (`technical/0006`, criterio sin marcar al cerrarla) | Que la cabecera cambie de contenido —marca o destinos—, o que el desplazamiento a 320px estorbe de verdad al usarla |
