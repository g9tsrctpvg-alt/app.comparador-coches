...
| Documentación: un recuento obsoleto en `scoreGap.ts`, a la espera de una spec que toque ese fichero | *Deudas abiertas* |
...
| Un recuento obsoleto, de los dos que registró el repaso del 2026-09-06. **Cerrado el de `fiabilidad`**: `product/0041` reescribió esa sección de `docs/estado/dominio.md` y el «nueve de los once candidatos» desapareció sin inventar ninguna cuenta — la frase dice ahora «la mayoría», que es lo que se puede afirmar sin el ranking completo de las 39 marcas, que el catálogo sigue sin guardar. **Sigue abierto** el de `src/domain/scoring/scoreGap.ts` (línea 145): un comentario habla de «las siete entradas de `AXIS_ORDER`», que desde `product/0037` son ocho. `product/0041` tocó `scoring/` con spec detrás y aun así no lo corrigió, porque su alcance era el eje de fiabilidad y ese comentario está en el reparto de la diferencia | 2026-09-06 | El comentario, en el próximo cambio con spec que toque `scoreGap.ts`; el recuento exacto de marcas, si alguien recupera el ranking OCU completo |
