...
| P16 — el cofre de techo parte el eje de espacio en dos: `product/0033` y `product/0034`, las dos en `draft`, a la espera del gate humano | *Propuestas de evolución*, abajo |
...
| P16 | **El cofre de techo rompe el 0,5 del maletero en `viaje`**: el eje pesa el maletero el doble que la batalla y que la anchura de hombros porque es «la restricción que se incumple —el equipaje cabe o no cabe—», y con un cofre en la baca deja de incumplirse: la carga de viaje pasa a ser comprable, la habitabilidad de atrás no | `en spec` | `product/0033` parte `viaje` en `carga` y `habitabilidad` —siete ejes, los seis anclajes intactos, pesos por defecto 5 y 5 porque son los únicos que dejan la clasificación bit a bit igual (la demostración, en su requisito 4.2)— y `product/0034` declara la carga máxima sobre el techo como magnitud de ficha, para que bajar el peso de la carga no premie a un coche que no admite baca. Las dos en `draft` el 2026-08-31, esperando gate. **Lo medido que sostiene la decisión:** maletero y habitabilidad son casi independientes (r = 0,28), así que hoy un solo peso —el más alto de los seis— gobierna dos cosas que no van juntas; sumar los litros del cofre al dato de maletero se descartó con medición (con +400 L la desviación de esa subnota cae de 1,26 a 0,22 y los dieciocho se apilan en el 10, el fallo del ADR 0010 al revés); y mover el reparto interno, que es aritméticamente equivalente a partir el eje y más barato, se descartó por producto y no por cálculo — la decisión se toma en la interfaz, y un supuesto escondido en un panel no es donde se decide cuánto pesa el maletero |
